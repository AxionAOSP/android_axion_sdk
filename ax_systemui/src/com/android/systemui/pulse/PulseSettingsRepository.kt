...
class PulseSettingsRepository @Inject constructor(
    ...
    private fun readSettings(): PulseSettings {
        val enabledDef = if (DEFAULT_ENABLED) 1 else 0
        val roundedDef = if (DEFAULT_ROUNDED_BARS) 1 else 0
        val isEnabled = secureSettings.getInt(PULSE_ENABLED, enabledDef) == 1
        val barCount = secureSettings.getInt(PULSE_BAR_COUNT, DEFAULT_BAR_COUNT).coerceIn(8, 64)
        val roundedBars = secureSettings.getInt(PULSE_ROUNDED_BARS, roundedDef) == 1
        val colorMode = PulseColorMode.fromKey(secureSettings.getString(PULSE_COLOR))
        val style = PulseStyle.fromId(secureSettings.getInt(PULSE_VIEW_STYLE, DEFAULT_STYLE))
        val settings = PulseSettings(isEnabled, barCount, roundedBars, colorMode, style)
        return settings
    }
}
