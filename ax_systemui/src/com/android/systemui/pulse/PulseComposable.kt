...
fun PulseVisualizer(
    ...
    if (!state.isEnabled || !state.isVisible || state.alpha <= 0f) return

    val density = LocalDensity.current
    val gapPx = with(density) { PulseConstants.BAR_GAP.toPx() }
    val maxHeightPx = with(density) { PulseConstants.MAX_HEIGHT.toPx() }

    val barHeights = remember(state.barCount) { BarHeightsState(state.barCount) }
    ...
    Canvas(modifier = modifier.fillMaxSize()) {
        if (frameCount.longValue < 0L) return@Canvas
        ...
        val barColor = when (state.colorMode) {
            PulseColorMode.LAVALAMP -> {
                ...
                val alphaInt = (PulseConstants.BAR_ALPHA * state.alpha * 255).toInt()
                Color(AndroidColor.HSVToColor(alphaInt, hsv))
            }
            else -> Color(state.barColor).copy(alpha = PulseConstants.BAR_ALPHA * state.alpha)
        }

        when (state.style) {
            PulseStyle.FADING_BLOCKS -> drawFadingBlocks(
                state, barHeights.currentHeights, barColor, gapPx, maxHeightPx, currentTime
            )
            PulseStyle.SOLID_LINE -> drawSolidLine(
                state, barHeights.currentHeights, barColor, maxHeightPx
            )
            else -> drawBars(
                state, barHeights.currentHeights, barColor, gapPx, maxHeightPx
            )
        }
    }
}
...
private fun DrawScope.drawFadingBlocks(
    ...
    for (i in 0 until barCount) {
        ...
        for (block in 0 until activeBlocks) {
            ...
            val fadeAlpha = (1f - blockRatio * 0.7f) * PulseConstants.BAR_ALPHA * state.alpha
