...
class PulseAudioDataProcessor @Inject constructor(
    ...
    private fun stopCaptureLocked() {
        if (!isProcessing && visualizer == null) return
    ...
    fun isCapturing(): Boolean = synchronized(lock) { isProcessing }
    ...
    private fun processFFTData(fftBytes: ByteArray, samplingRate: Int) {
        ...
        synchronized(lock) {
            ...
            val currentBands = melBands
            val currentCenters = bandCenterHz
            val (bands, centers) = if (currentBands != null &&
                currentCenters != null &&
                currentBands.size == barCount &&
                currentCenters.size == barCount &&
                numBins == lastNumBins
            ) {
                Pair(currentBands, currentCenters)
            } else {
                val built = buildMelBands(barCount, numBins, binHz)
                melBands = built.first
                bandCenterHz = built.second
                lastNumBins = numBins
                built
            }

            val currentPeaks = perBandPeak
            val peaks = if (currentPeaks == null || currentPeaks.size != barCount) {
                FloatArray(barCount) { PEAK_FLOOR }.also { perBandPeak = it }
            } else {
                currentPeaks
            }

            val currentSmoothed = smoothed
            val prev = if (currentSmoothed == null || currentSmoothed.size != barCount) {
                FloatArray(barCount).also { smoothed = it }
            } else {
                currentSmoothed
            }
