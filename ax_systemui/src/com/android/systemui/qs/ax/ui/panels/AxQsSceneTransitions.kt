...
fun TransitionBuilder.axFromQuickQuickSettingsToQuickSettings() {
    fractionRange(end = AX_QS_SCENE_FADE_START) {
        fade(AxSceneKeys.QuickQuickSettings.rootElementKey)
    }
...
fun TransitionBuilder.toAxEditMode() {
    fade(AxSceneKeys.EditMode.rootElementKey)
    scaleDraw(
        AxSceneKeys.EditMode.rootElementKey,
        scaleX = 0.85f,
        scaleY = 0.85f,
    )
    ...
    fade(AxSceneKeys.QuickQuickSettings.rootElementKey)
    scaleDraw(
        AxSceneKeys.QuickQuickSettings.rootElementKey,
        scaleX = 0.85f,
        scaleY = 0.85f,
    )
    disableAxQsSharedElements()
...
fun SceneTransitionLayoutState.editModeTransitionProgress(): Float {
    return when (val state = transitionState) {
        is TransitionState.Idle -> {
            if (state.currentScene == AxSceneKeys.EditMode || state.currentScene == AxSceneKeys.PanelSettings) {
                1f
            } else {
                0f
            }
        ...
        is TransitionState.Transition -> {
            val toEdit = state.toContent == AxSceneKeys.EditMode || state.toContent == AxSceneKeys.PanelSettings
            val fromEdit = state.fromContent == AxSceneKeys.EditMode || state.fromContent == AxSceneKeys.PanelSettings
            when {
...
private fun ContentKey.isAxQsScene(): Boolean {
    return this == AxSceneKeys.QuickSettings || this == AxSceneKeys.QuickQuickSettings
}
