...
class PulseDisplayRepository @Inject constructor(
    ...
    private val displayManager = checkNotNull(context.getSystemService(DisplayManager::class.java))
    ...
    private fun computeDisplayState(): PulseDisplayState {
        val display = displayManager.getDisplay(Display.DEFAULT_DISPLAY) ?: context.display
        val rawRate = display?.refreshRate ?: 60f
        val refreshRate = if (rawRate > 0f && !rawRate.isNaN()) rawRate else 60f
        val throttleMs = (1000f / refreshRate).toLong().coerceIn(8L, 100L)
        val state = display?.state ?: Display.STATE_ON
        val screenOn = state != Display.STATE_OFF

        return PulseDisplayState(refreshRate, throttleMs, screenOn)
    }
}
